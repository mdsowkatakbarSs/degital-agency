...
// Platform SVG icons matching the existing design
...
export function GigCard({ gig, index = 0 }: GigCardProps) {
  ...
  const minPrice = gig.gig_packages?.length
    ? Math.min(...gig.gig_packages.map((p) => p.price))
    : gig.starting_price;
          ...
          <div className="p-5">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-lg font-bold">{formatPrice(minPrice)}</div>
              </div>
              <div className="flex items-center gap-1 text-xs text-muted-foreground">
                <Clock className="w-3.5 h-3.5" />
                {formatDeliveryDays(gig.delivery_days)}
              </div>
            </div>
          </div>
        </div>
